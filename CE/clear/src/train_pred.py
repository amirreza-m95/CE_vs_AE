...
parser.add_argument('--dataset', default='imdb_m', help='dataset to use',  #ogbg_molhiv
                    choices=['community', 'ogbg_molhiv', 'imdb_m'])
parser.add_argument('--lr', type=float, default=0.001,  #default=1e-3
...
def test(params):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        features = data['features'].float().to(device) #shape: batch_size, 15, 2
        adj = data['adj'].float().to(device)  #shape: batch_size, 15, 15
        u = data['u'].float().to(device) # (batch_size, 1)
        labels = data['labels'].float().to(device)
