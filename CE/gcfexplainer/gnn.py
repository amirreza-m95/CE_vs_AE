...
def load_trained_gnn(dataset_name, device):
    ...
    model.load_state_dict(torch.load(os.path.join(sys.path[0] ,f'data/{dataset_name}/gnn/model_best.pth'), map_location=device), strict=False)
    return model


@torch.no_grad()
def load_trained_prediction(dataset_name, device):
    prediction_file = os.path.join(sys.path[0] ,f'data/{dataset_name}/gnn/preds.pt')
...
def load_trained_embeddings_logits(dataset_name, device):
    node_embeddings_file = os.path.join(sys.path[0] ,f'data/{dataset_name}/gnn/node_embeddings.pt')
    graph_embeddings_file = os.path.join(sys.path[0] ,f'data/{dataset_name}/gnn/graph_embeddings.pt')
    logits_file = os.path.join(sys.path[0] ,f'data/{dataset_name}/gnn/logits.pt')
...
def main():
    ...
    # Logging.
    gnn_folder = os.path.join(sys.path[0] ,f'data/{args.dataset}/gnn/')
